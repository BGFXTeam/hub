...
local ToastText
local ToastTimer
function BGFX.showToast(content)
--print("ZevToasterOP('" .. content .."')")
pcall(BGFX.a, content)
end
function BGFX.a(content)
    content = "ZevToasterOP('" .. content .."')"
    if ToastText == nil then
        ...
        ToastText:SetProperty("BackgroundColor", "0 0 0 0.4")
...
function KeyBoardI.onKeyUp(keyName, keyCode)
	...
	UIHelper.showToast("Panel Open")
	if A then
    GUIGMControlPanel:hide()
	UIHelper.showToast("Panel Closed")
    end
	return
	end
    if keyName == "R" then
...
GMSetting:addItem("^FFFF00Hack", "ESP", "EspMode")
GMSetting:addItem("^FFFF00Hack", "TestCCall", "TestCallC")
...
function GMHelper:ChangeTime(isNight)
   ...
   if isNight then
	  ...
	  UIHelper.showToast("Now Night!")
      return
   end
   curWorld:setWorldTime(6000)
   UIHelper.showToast("Now Day!")
   end
...
function GMHelper:StartTime()
   ...
   if isTimeStopped then
     UIHelper.showToast("Start/Stop Time: disabled!")
     return
   end
   UIHelper.showToast("Start/Stop Time: enabled!")
end
...
function GMHelper:MyLoveFly(text)
   ...
   if A then
	UIHelper.showToast("FLy ON")
     return
   end
    ClientHelper.putBoolPrefs("EnableDoubleJumps", false)
	UIHelper.showToast("FLy OFF")
end
...
function GMHelper:ViewBobbing()
   ...
   if A then
	UIHelper.showToast("ViewBobbing: OFF")
     return
   end
    ClientHelper.putBoolPrefs("IsViewBobbing", true)
	UIHelper.showToast("ViewBobbing: ON")
end
...
function GMHelper:BlockmanCollision()
   ...
   if A then
	UIHelper.showToast("BlockmanCollision: ON")
     return
   end
    ClientHelper.putBoolPrefs("IsBlockmanCollision", false)
	UIHelper.showToast("BlockmanCollision: OFF")
...
function GMHelper:Fog()
   ...
   if A then
	UIHelper.showToast("Fog Disabled!")
     return
   end
    ClientHelper.putBoolPrefs("DisableFog", false)
	UIHelper.showToast("Fog Enabled!")
end
...
function GMHelper:WWE_Camera()
   ...
   if A then
	UIHelper.showToast("SeparateCamera: Enabled")
     return
   end
...
function GMHelper:CloudsOFF()
	...
	UIHelper.showToast("Clouds Stop")
	GUIGMControlPanel:hide()
end
...
function GMHelper:BowSpeed()
	...
	UIHelper.showToast("BowSpeed:ON")
	GUIGMControlPanel:hide()
end
...
function GMHelper:HeadText()
   ...
   if A then
	UIHelper.showToast("Head text render now ON")
     return
   end
   ClientHelper.putBoolPrefs("RenderHeadText", false)
	UIHelper.showToast("Head text render now OFF")
end
...
function GMHelper:BanClickCD()
   A = not A
    ...
    UIHelper.showToast("Nodelay ON!")
   if A then
    ClientHelper.putBoolPrefs("banClickCD", false)
	UIHelper.showToast("Nodelay OFF!")
end
end
...
function GMHelper:SpeedLineMode()
	...
	UIHelper.showToast("Speed Line = Enable!")
	GUIGMControlPanel:hide()
end
...
function GMHelper:SpeedLineModeDisable()
    ...
    local interval = 0
    Blockman.Instance().m_gameSettings:setPatternSpeedLine(strength, interval)
	UIHelper.showToast("Speed Line = Disabled!")
	GUIGMControlPanel:hide()
end
...
function GMHelper:PatternTorchMode()
	...
	UIHelper.showToast("PatternTorch = Enabled!")
	GUIGMControlPanel:hide()
end
...
function GMHelper:PatternTorchModeOFF()
    local strength = 0
    Blockman.Instance().m_gameSettings:setPatternTorchStrength(strength)
	UIHelper.showToast("PatternTorch = Disabled!")
	GUIGMControlPanel:hide()
end
...
function GMHelper:BlinkOP()
   ...
   if A then
	UIHelper.showToast("Blink Enabled!")
     return
   end
    ClientHelper.putBoolPrefs("SyncClientPositionToServer", true)
	UIHelper.showToast("Blink Disabled!")
end
...
function GMHelper:FlyDev2()
	...
	UIHelper.showToast("Speed Break Block = 0")
	GUIGMControlPanel:hide()
end
end
end
...
function GMHelper:Noclip()
   ...
   if A then
	UIHelper.showToast("Noclip = true")
     return
   end
    for blockId = 1, 40000 do
        local block = BlockManager.getBlockById(blockId)
	...
	UIHelper.showToast("Noclip = false")
end
...
function GMHelper:NoclipOP()
   ...
   if A then
	UIHelper.showToast("TreasureHunterNoClip = Enabled")
     return
   end
    for blockId = 3, 133 do
        local block = BlockManager.getBlockById(blockId)
	...
	UIHelper.showToast("TreasureHunterNoClip = Disabled")
end
...
function GMHelper:ChangeNick()
   GMHelper:openInput({ "" }, function(Nick)
    ...
    UIHelper.showToast("NickNameChanged")
   end)
end
...
function GMHelper:SharpFly()
    ...
    UIHelper.showToast("[ON] works when you have dev flight enabled")
	if A then
	ClientHelper.putBoolPrefs("DisableInertialFly", false)
    UIHelper.showToast("[OFF] works when you have dev flight enabled")
end
end
...
function GMHelper:GetAllInfoT()
    local players = PlayerManager:getPlayers()
    for _, player in pairs(players) do
    MsgSender.sendMsg("INFO: " .. string.format("UserName: %s {} ID: %s {} Gender: %s", player:getName(), player.userId, player.Player:getSex()))
end
end
...
function GMHelper:spawnCar()
  ...
  Blockman.Instance():getWorld():addVehicle(pos, ID, yaw)
	UIHelper.showToast("Car Spawn Success")
end)
end
        ...
        LuaTimer:scheduleTimer(function()
            ...
            for _, nubs in pairs(hoomans) do
                if nubs.userId ~= myclient.userId and nubs.Player then
                    -- Force color update by toggling XRay
                    nubs.Player:disableXRay()
                    nubs.Player:enableXRay(rainbowColor)
                end
            end
        end, 1) -- update every 1 tick (adjust as needed)
    else
        local hoomans = PlayerManager:getPlayers()
        for _, nubs in pairs(hoomans) do
            if nubs.Player then
...
function GMHelper:TestCallC()
loadstring("--[[Toaster('Hi World')]]")()
pcall(load("--[[Toaster('Hi World')]]"))
BGFX.showToast("CALLED ASF")
end
